import { TileType } from "../game_board";
...
export class WaveExecutor {
    ...
    creepsRemoved;
    projectileTracker;
    constructor(board, wave, orchestrator) {
        this.board = board;
        this.wave = wave;
        this.nextCreepIdx = 0;
    ...
    moveActiveCreep(creep) {
        let walkingPaths = this.board.getWalkingBestPathSegments();
        let activeSegment = walkingPaths[creep.checkpointsCrossed];
        let currentCoord = getCurrentTile(creep.pxPos, this.board.config.tilePixelDim);
        let currentIndexInSegment = activeSegment.findIndex(coord => coord.col === currentCoord.col && coord.row === currentCoord.row);
    ...
    stepFrameForTowers() {
        this.board.towers.forEach((tower) => {
            ...
            if (tower.targettedCreep && tower.framesReloading <= 0) {
                this.projectileTracker.addProjectile(new Projectile(tower, tower.targettedCreep, tower.type.projectileConfig));
                //TODO effective value.
                tower.framesReloading = tower.type.baseFramesPerAttk;
            }
        });
    }
    tryToTargetCreep(tower, activeCreeps) {
        let targetableCreep = firstInRange(tower, activeCreeps, this.board.config.tilePixelDim);
        if (targetableCreep)
            tower.targettedCreep = targetableCreep;
    }
}
export const creepIsInRange = (tower, creep, dim) => {
    //TODO effective.
    return calculateDistance(getTileCenterPx(tower.pos, dim), creep.pxPos)
        <= tower.type.baseRangePx;
};
export const firstInRange = (tower, activeCreeps, dim) => {
    return activeCreeps.filter((creep) => creepIsInRange(tower, creep, dim))?.[0];
};
