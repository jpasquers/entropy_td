...
export class AddTowerActor extends ActionBridge<ActiveGameScene> implements ClickObserver {
    ...
    onEvent(event: ClickEvent): void {
        if (this.scene.terrainRenderer!.isPixelRelated(event.targetWorldPos)) {
            let coord = this.scene.terrainRenderer!.getTileCoordForRenderedPixel(event.targetWorldPos);
